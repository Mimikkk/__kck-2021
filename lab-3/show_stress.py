...
def tuple3xByteToString(rgb: Tuple[int, int, int]) -> str:
  (r, g, b) = rgb
...
def plot(arr):
  ...
  for i in range(len(arr)):
    ...
    line = canvas.create_line(getX(x1, y1, z1), getY(x1, y1, z1), getX(x2, y2, z2), getY(x2, y2, z2), width=2,
                              fill=tuple3xByteToString(rgb))
